...
public class TenantDatabaseUpgradeService {
    ...
    public void upgradeAllTenants() {
        upgradeTenantDB();
        MifosPlatformTenantConnection connection = null;
        ...
        for (final MifosPlatformTenant tenant : tenants) {
            if (ThreadLocalContextUtil.isReportRequest() != null && ThreadLocalContextUtil.isReportRequest()) {
                connection = tenant.getReportsConnection();
            } else {
                connection = tenant.getOltpConnection();
            }
            if (connection != null && connection.isAutoUpdateEnabled()) {
