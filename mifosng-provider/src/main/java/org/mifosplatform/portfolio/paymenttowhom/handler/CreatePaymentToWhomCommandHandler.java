...
@Service
@CommandType(entity = "PAYMENTTOWHOM" ,action = "CREATE")
public class CreatePaymentToWhomCommandHandler implements NewCommandSourceHandler{

	private final PaymentToWhomWriteService paymentToWhomWriteService;
	
	@Autowired
	public CreatePaymentToWhomCommandHandler(final PaymentToWhomWriteService paymentToWhomWriteService){
		this.paymentToWhomWriteService = paymentToWhomWriteService;
	}
	@Override
	@Transactional
	public CommandProcessingResult processCommand(JsonCommand command){
		return this.paymentToWhomWriteService.createPaymentToWhom(command);
	}
}
