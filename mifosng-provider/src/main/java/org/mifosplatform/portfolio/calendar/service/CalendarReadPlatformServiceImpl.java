...
public class CalendarReadPlatformServiceImpl implements CalendarReadPlatformService {
    ...
    public Collection<LocalDate> generateRecurringDates(final CalendarData calendarData, final boolean withHistory, final LocalDate tillDate) {
        ...
        Collection<LocalDate> recurringDates = generateRecurringDate(calendarData, tillDate, -1);
        ...
        if (withHistory) {
            ...
            for (CalendarData calendarHistory : calendarHistorys) {
                recurringDates.addAll(generateRecurringDate(calendarHistory, tillDate, -1));
            }
        }
    ...
    public Collection<LocalDate> generateNextTenRecurringDates(CalendarData calendarData) {
        return generateRecurringDate(calendarData, null, 10);
    }

    private Collection<LocalDate> generateRecurringDate(final CalendarData calendarData, final LocalDate tillDate, final int maxCount) {
        ...
        final LocalDate periodStartDate = this.getPeriodStartDate(seedDate, calendarData.getStartDate());
    ...
    private LocalDate getPeriodStartDate(LocalDate seedDate, LocalDate startDate) {
    ...
    public Collection<CalendarData> updateWithRecurringDates(final Collection<CalendarData> calendarsData) {
        ...
        for (final CalendarData calendarData : calendarsData) {
            final Collection<LocalDate> recurringDates = this.generateRecurringDates(calendarData, withHistory, null);
