...
public class DepositAccountOnHoldTransactionReadPlatformServiceImpl implements
	...
	@Autowired
	public DepositAccountOnHoldTransactionReadPlatformServiceImpl(
	...
	@Override
	public Page<DepositAccountOnHoldTransactionData> retriveAll(Long savingsId,
			Long guarantorFundingId, SearchParameters searchParameters) {
		...
		return this.paginationHelper.fetchPage(this.jdbcTemplate, sqlCountRows,
				sqlBuilder.toString(), finalObjectArray, this.mapper);

	}

	private static final class DepositAccountOnHoldTransactionsMapper implements
			RowMapper<DepositAccountOnHoldTransactionData> {
		...
		public DepositAccountOnHoldTransactionsMapper() {
			...
			sqlBuilder
					.append(" left join m_guarantor_transaction gt on gt.deposit_on_hold_transaction_id = tr.id ");
			sqlBuilder
					.append(" left join m_guarantor_funding_details mgfd on mgfd.id=gt.guarantor_fund_detail_id");
			sqlBuilder
					.append(" left join m_portfolio_account_associations  pa on pa.id=mgfd.account_associations_id");
			sqlBuilder
					.append(" left join m_loan ml on ml.id = pa.loan_account_id");
		...
		public DepositAccountOnHoldTransactionData mapRow(final ResultSet rs,
			...
			final int transactionTypeInt = JdbcSupport.getInteger(rs,
					"transactionType");
			final EnumOptionData transactionType = SavingsEnumerations
					.onHoldTransactionType(transactionTypeInt);
