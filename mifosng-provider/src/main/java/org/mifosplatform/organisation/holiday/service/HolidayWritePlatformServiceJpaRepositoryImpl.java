...
public class HolidayWritePlatformServiceJpaRepositoryImpl implements HolidayWritePlatformService {
    ...
    private void validateInputDates(final JsonCommand command) {
        ...
        if(alternativeWorkingDate.isBefore(fromDate.minusDays(7)) || alternativeWorkingDate.isAfter(toDate.plusDays(7))){
            final String defaultUserMessage = "Alternative working date must be within 7 days before or after from and to dates";
            throw new HolidayDateException("alternative.working.date.must.be.within.range", defaultUserMessage, fromDate.toString(), toDate.toString(), alternativeWorkingDate.toString());
        }
    }
}
