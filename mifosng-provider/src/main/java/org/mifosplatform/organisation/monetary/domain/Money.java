...
public class Money implements Comparable<Money> {
    ...
    public static Money of(final MonetaryCurrency currency, final BigDecimal newAmount) {
        return new Money(currency.getCode(), currency.getDigitsAfterDecimal(), defaultToZeroIfNull(newAmount),
                currency.getMultiplesofDecimal());
    }

    public static Money zero(final MonetaryCurrency currency) {
        return new Money(currency.getCode(), currency.getDigitsAfterDecimal(), BigDecimal.ZERO, currency.getMultiplesofDecimal());
    }
    ...
    private Money(final String currencyCode, final int digitsAfterDecimal, final BigDecimal amount, final int multiplesofDecimal) {
        ...
        if (this.currencyDigitsAfterDecimal == 0 && multiplesofDecimal>0 && amountScaled.doubleValue()>0) {
            double existingVal = amountScaled.doubleValue();
            double ceilingOfValue = ceiling(existingVal, multiplesofDecimal);
            double floorOfValue = floor(existingVal, multiplesofDecimal);
    ...
    public Money copy() {
        return new Money(this.currencyCode, this.currencyDigitsAfterDecimal, this.amount.stripTrailingZeros(), this.multiplesofDecimal);
    }
    ...
    public int getCurrencyMultiplesofDecimal() {
    ...
    private MonetaryCurrency monetaryCurrency() {
        return new MonetaryCurrency(this.currencyCode, this.currencyDigitsAfterDecimal, this.multiplesofDecimal);
    }
